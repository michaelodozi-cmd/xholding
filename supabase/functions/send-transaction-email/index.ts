...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const configuredSecret = Deno.env.get("WEBHOOK_SECRET") || "Fidelity Holdings-webhook-secret-2026";
    ...
    if (type === 'INSERT') {
      if (txType === 'withdrawal') {
        shouldSend = true;
        subject = "Withdrawal Request Pending - Fidelity Holdings";
        html = `
          <div style="font-family: 'Inter', sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 12px; background-color: #ffffff; color: #1a202c;">
            ...
            <p style="font-size: 13px; color: #718096; text-align: center; margin: 0;">This is an automated message from Fidelity Holdings. Please do not reply directly to this email.</p>
          </div>
        `;
        ...
        if (['PROFIT', 'BONUS', 'ADJUSTMENT'].includes(asset)) {
          ...
          subject = `${actionName} - Fidelity Holdings`;
          html = `
            <div style="font-family: 'Inter', sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 12px; background-color: #ffffff; color: #1a202c;">
          ...
          subject = "Deposit Request Received - Fidelity Holdings";
          html = `
            <div style="font-family: 'Inter', sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 12px; background-color: #ffffff; color: #1a202c;">
      ...
      if (oldStatus !== newStatus && (newStatus === 'approved' || newStatus === 'rejected')) {
        ...
        subject = `${capitalizedType} Request ${isApproved ? 'Approved' : 'Rejected'} - Fidelity Holdings`;
        html = `
          <div style="font-family: 'Inter', sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 12px; background-color: #ffffff; color: #1a202c;">
    ...
    if (shouldSend) {
      ...
      if (!resendApiKey) {
        ...
        const response = await fetch("https://api.resend.com/emails", {
          ...
          body: JSON.stringify({
            from: `Fidelity Holdings <${senderEmail}>`,
            to: [userEmail],
            subject: subject,
            html: html
          })
